...
def initialize_train_network(X_train, Y_train):
...
def initialize_test_network(X_test, Y_test, layers):
...
def train_Network(X_train, Y_train):
    layers, m, Y = initialize_train_network(X_train, Y_train)

    cost_curve = []

    for i in range(1, iterations):
        print(i)
        layers = forward_propagate(layers)
        layers = back_propagate(layers, m, Y)
        cost = -1 / m * np.sum(
...
def test_Network(X_test, Y_test, layers):
    layers, m, Y = initialize_test_network(X_test, Y_test, layers)
    layers = forward_propagate(layers)
    cost = -1 / m * np.sum(
        (Y * np.log(layers['A' + str(output_layer)])) + (1 - Y) * np.log(1 - layers['A' + str(output_layer)]))

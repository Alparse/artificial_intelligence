...
with graph1.as_default():
    ...
    #Theta4 = tf.Variable(tf.random_uniform([9, 9], -1, 1), name="Theta4", trainable=True)  # Tensor, variable
    ...
    Bias3 = tf.Variable(tf.zeros([9]), name="Bias2", trainable=True)  # Tensor, variable
    #Bias4 = tf.Variable(tf.zeros([9]), name="Bias2", trainable=True)  # Tensor, variable
...
with tf.Session(graph=graph1) as sess:
    ...
    for i in range(125000):
        ...
        while game.game_status == 0:
            if game.turn in[2,4,6,8]:
                ...
                if game.check_if_game_won(player):
                    ...
                    game.reward_dictionary1[reward_dict_size-1][0]=-1
                    game.game_status=2
        ...
        if game.game_status > 0:
            ...
            if game.game_status == 1:
                #duration_multiplier = 1
                duration_multiplier=9/game.turn

            print("\n", game.reward_dictionary1, "\n")
            for rew_ind, rewards33 in enumerate(reversed(game.reward_dictionary1)):
                rewards33[0] = rewards33[0] * (discount_rate ** (rew_ind+1))
